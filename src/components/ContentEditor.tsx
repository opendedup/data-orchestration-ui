...
export default function ContentEditor({
  ...
  if (!selectedItem) {
    return (
      <div
        className="h-full flex items-center justify-center text-gray-100"
        style={{ backgroundColor: "var(--surface-panel)" }}
      >
        <div className="text-center text-gray-200">
          <Eye className="w-16 h-16 mx-auto mb-4 opacity-60" />
          <p className="text-lg font-medium text-gray-100">No item selected</p>
          <p className="text-sm mt-2 text-gray-200">Select an item from the tree to view or edit</p>
        </div>
      </div>
    );
  }
  ...
  const renderMetadata = () => {
    ...
    return (
      <div
        className="border-b border-gray-700 p-4 text-gray-100"
        style={{ backgroundColor: "var(--surface-panel)" }}
      >
          ...
          <div className="flex items-center gap-2">
            ...
            {!isEditing ? (
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={handleSave}
                  className="flex items-center gap-1 px-3 py-1.5 text-sm bg-gray-200 hover:bg-gray-100 text-gray-900 rounded transition-colors"
                >
        ...
        {selectedType === "queries" && (
          <div className="space-y-2">
            <div>
              <label className="text-xs text-gray-200 block mb-1">Question</label>
              {isEditing ? (
                <input
                  type="text"
                  ...
                  className="w-full px-2 py-1 text-sm bg-gray-800 text-gray-100 rounded border border-gray-600 focus:border-blue-400 focus:outline-none"
                />
              ) : (
                <p className="text-sm text-gray-100">{item.question}</p>
              )}
            </div>
            <div>
              <label className="text-xs text-gray-200 block mb-1">Summary</label>
              {isEditing ? (
                <input
                  type="text"
                ...
                <p className="text-sm text-gray-100">{item.querysummary}</p>
              )}
            </div>
          </div>
        )}
        ...
        {selectedType === "datasets" && (
          <div>
            <label className="text-xs text-gray-200 block mb-1">Table ID</label>
            <p className="text-sm text-gray-100 font-mono">{item.table_id}</p>
          </div>
        )}

        {selectedType === "query_results" && (
          <div>
            <label className="text-xs text-gray-200 block mb-1">Query</label>
            <p className="text-sm text-gray-100 font-mono truncate">{item.query}</p>
          </div>
        )}
      </div>
    );
          ...
          <div className="prose prose-invert max-w-none text-gray-100">
            {selectedType === "queries" ? (
              <pre className="bg-gray-900 p-4 rounded-lg text-sm overflow-x-auto border border-gray-700">
                <code className="text-gray-100">{editedContent}</code>
              </pre>
            ) : (
              <ReactMarkdown
                ...
                components={{
                  code: ({ node, className, children, ...props }) => {
                    ...
                    return isInline ? (
                      <code className="bg-gray-900 px-1 py-0.5 rounded text-sm text-gray-100" {...props}>
                        {children}
                      </code>
                    ) : (
                  ...
                  table: ({ node, ...props }) => (
                    <div className="overflow-x-auto">
                      <table className="min-w-full border border-gray-700 text-gray-100" {...props} />
                    </div>
                  ),
                  th: ({ node, ...props }) => (
                    <th className="border border-gray-700 px-4 py-2 bg-gray-900 text-gray-100" {...props} />
                  ),
                  td: ({ node, ...props }) => (
                    <td className="border border-gray-700 px-4 py-2 text-gray-100" {...props} />
                  ),
                }}
              >
                {editedContent}
